...
import { InstallAgentApp, log } from "./common";
...
module.exports = async (orchestrator) => {
    orchestrator.registerScenario("try to retieve a profile that doesnt exist, get fields needed, create one and try again", async (s, t) => {
        ...
        const alice_cell = await InstallAgentApp(s, "alice-cell");
         ...
         let my_agent_key = await alice_cell.call("profiles", "who_am_i", null);
        log("agent key should be",my_agent_key)
        log("agent_key", alice_cell.cellId[0].toString("base64"));
        log("cellId", alice_cell.cellId[1].toString("base64"));
   ...
   console.log("--------------------------------------------------------")
        ...
        log("TEST1: get profile with app_info (app to zome call):",appInfo)
        try {
           let profile = await alice_cell.call(
             "profiles",
             "get_profile",
             appInfo
           );
    ...
    try {
      let persona = await alice_cell.call(
        "personas",
        "get_persona"
      );
    ...
    try {
      let non_existing_Fields = await alice_cell.call(
        "personas",
        "get_fields",      
        { 
          fields:field_set
  ...
  //returns personaField
    ...
    try{
      let pFields = await alice_cell.call(
        "personas",
        "add_field",
        {
  ...
  */
    ...
    let profileHash = await alice_cell.call(
      "profiles",
      "create_profile",
      appInfo
    );
